    ...
    async fn token_server(refresh_rotates: bool) -> String {
        let app = Router::new().route(
            ...
            post(move |body: axum::body::Bytes| async move {
                ...
                let is_refresh = raw.contains("grant_type=refresh_token");
                if !raw.contains("client_secret=test-antigravity-client-secret")
                    || (!is_refresh && !raw.contains("code_verifier="))
                {
                    return (
                        StatusCode::BAD_REQUEST,
                ...
                if is_refresh && raw.contains("bad-refresh") {
                    return (
                        StatusCode::BAD_REQUEST,
